...
from utils import (sci_round, mmHg_2_kPa, kPa_2_mmHg, one_D_matching, draw,
                   temp_to_K, get_rou_by_t, get_miu_by_t, get_sita_by_t,
                   get_C_20, D_round, get_c_p_by_t, get_latent_heat_t,
                   get_p_by_temp, get_temp_by_p, kgf_to_mmHg, ge_y_by_x,
                   get_value_of_key)
import numpy as np
...
class Work:
    ...
    def get_thero_plates(self, R_multiple=1.2):
        ...
        k = sci_round(R / (R + 1), 4)
        c = sci_round(material.x_D / (R + 1), 4)
        y_inter_q = sci_round(k * material.x_F + c, 4)
        k_h = sci_round(
            (y_inter_q - material.x_w) / (material.x_F - material.x_w), 4)
        c_h = sci_round(material.x_w - k_h * material.x_w, 4)
        x = []
        y = []
        y_current = material.x_D
        X, Y = one_D_matching(np.array(material.phase_equilibrium['x']),
                              np.array(material.phase_equilibrium['y']))

        def get_x_current(y_current):
            # print('get', y_current)
            return sci_round(ge_y_by_x(Y, X, y_current), 4)
        ...
        while x_current >= material.x_w:
            if x_current >= material.x_F:
                y_current = sci_round(k * x_current + c, 4)
                x_current = get_x_current(y_current)
                N_r += 1
            else:
                y_current = sci_round(k_h * x_current + c_h, 4)
                x_current = get_x_current(y_current)
            x.append(x_current)
            y.append(y_current)
            if y[-1] == y[-2]:
        ...
        return {
            ...
            'stripping_operating_line':
            f"y={k_h}x{'+' if c_h > 0 else ''}{c_h}",
            'R': R,
    ...
    def get_real_plates(self, R_multiple):
        ...
        return {
            'N_real': N / E_T,
            'N_real_rounded': math.ceil(N / E_T),
    ...
    def get_M_v(self, y):
        ...
        return sci_round(y * M_solute + (1 - y) * M_solvent, 4)
    ...
    def get_tower_diameter(self, R_multiple, u_multiple, H_T=0.35, h_L=0.05):
        ...
        sita_solute = sci_round(get_sita_by_t(self.solute_sita_t, t_top), 4)
        sita_solvent = sci_round(get_sita_by_t(self.solvent_sita_t, t_top), 4)
        sita = sci_round(sita_solute * x_1 + sita_solvent * (1 - x_1), 4)
    ...
    def get_reboiler(self, R_multiple, loss_Q=1.05, K_reboiler=300):
        ...
        r_solvent = sci_round(get_latent_heat_t(self.solvent_latent_heat,
                                                t_in))
        M_solute = material.M_solute
        M_solvent = material.M_solvent
    ...
    def get_cost_R(self,
        ...
        return {
            'R': [sci_round(i, 4) for i in R],
            'cost': cost,
            'min': (R_min, cost_new[cost_min])
        }
